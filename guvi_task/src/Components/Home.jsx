...
const Home = () => {
  const { open, openCreate, search } = useDialogBox();
  ...
  // Filter notes based on tab and search input
    ...
    <div className="max-w-2xl mx-auto p-12">
        ...
        {TABS.map(({ key, label }) => (
          <button
            ...
            className={`px-6 py-2 rounded-full border font-bold ${
              tab === key ? "bg-yellow-400" : "bg-gray-100"
            }`}
            onClick={() => setTab(key)}
          >
      ...
      {notes.length === 0 ? (
        <div className="py-20 text-center text-xl text-gray-600 flex flex-col items-center">
          <svg width="50" height="60" fill="none" stroke="#a3a3a3">
            <rect width="40" height="50" x="5" y="5" rx="6" strokeWidth="2" />
            <line
              x1="12"
              y1="20"
              x2="33"
        ...
        <div className="space-y-6">
          {notesToShow.map((note) => (
            <div
              key={note.id}
              className="bg-white rounded-xl p-5 shadow flex flex-col gap-2"
            >
